...
contains
  ...
  subroutine patmo_run(dt,convergence)
    ...
    !atol(:) = 1000.0 !absolute tolerances (array)
    !rtol(:) = 1d-1 !relative tolerances (array)
    ...
    total_species = total_species + sum(nall(:,patmo_idx_32COS))
...
patmo_getTotalMassNuclei_O = getTotalMassNuclei_O() 
 ...
 !calculate atmospheric burden per icell
  function patmo_Tgyr(icell,idx)
   use patmo_constants
   use patmo_commons
   use patmo_parameters
   implicit none 
   integer,intent(in)::icell,idx
   real*8::patmo_Tgyr
   integer::i,j
   do i=1,cellsNumber       
     patmo_Tgyr = nall(i,idx)*surf_vol/av*meanMolecularMass*secondsPerYear/1d12
   enddo
   end function patmo_Tgyr
  subroutine patmo_dumpburden(icell,idx)
    use patmo_constants
   use patmo_commons
   use patmo_parameters
   implicit none
   ...
   real*8::trop_burden
   real*8::strat_burden
    trop_burden = sum(nall(1:12,idx))
    strat_burden = sum(nall(13:50,idx))   
   end subroutine patmo_dumpburden
end module patmo
